            ...
            if (id) {
                this.historyService.find(id)
                    .subscribe((historyResponse: HttpResponse<History>) => {
                        const history: History = historyResponse.body;
                        if (history.solvedDate) {
                            history.solvedDate = {
                                year: history.solvedDate.getFullYear(),
                                month: history.solvedDate.getMonth() + 1,
                                day: history.solvedDate.getDate()
                            };
                        }
                ...
                setTimeout(() => {
                    this.ngbModalRef = this.historyModalRef(component, new History());
                    resolve(this.ngbModalRef);
                }, 0);
            }
        });
