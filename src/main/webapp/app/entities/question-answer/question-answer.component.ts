...
currentAccount: any;
    ...
    loadAll() {
        if (this.currentSearch) {
            this.questionAnswerService.search({
                page: this.page - 1,
                query: this.currentSearch,
                size: this.itemsPerPage,
                sort: this.sort()}).subscribe(
                    (res: HttpResponse<QuestionAnswer[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/question-answer'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: QuestionAnswer) {
        return item.id;
    }
