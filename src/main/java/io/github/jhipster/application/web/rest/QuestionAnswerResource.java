...
public class QuestionAnswerResource {
    ...
    public QuestionAnswerResource(QuestionAnswerRepository questionAnswerRepository, QuestionAnswerSearchRepository questionAnswerSearchRepository) {
    ...
    @Timed
    public ResponseEntity<List<QuestionAnswer>> getAllQuestionAnswers(Pageable pageable) {
        log.debug("REST request to get a page of QuestionAnswers");
        Page<QuestionAnswer> page = questionAnswerRepository.findAll(pageable);
    ...
    /**
     * SEARCH  /_search/question-answers?query=:query : search for the questionAnswer corresponding
     * to the query.
     *
     * @param query the query of the questionAnswer search
     * @param pageable the pagination information
     * @return the result of the search
     */
    @GetMapping("/_search/question-answers")
    @Timed
    public ResponseEntity<List<QuestionAnswer>> searchQuestionAnswers(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of QuestionAnswers for query {}", query);
        Page<QuestionAnswer> page = questionAnswerSearchRepository.search(queryStringQuery(query), pageable);
        HttpHeaders headers = PaginationUtil.generateSearchPaginationHttpHeaders(query, page, "/api/_search/question-answers");
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
